...
def test_checkout_total(driver):
    ...
    with allure.step("Авторизоваться как пользователь "
                     "standard_user с паролём secret_sauce"):
    ...
    with allure.step(f"Проверка итоговой суммы. Ожидаемая сумма '$58.29', "
                     f"фактическая '{total}'"):
        assert total == "Total: $58.29", (f"Expected total to "
                                          f"be '$58.29', but got '{total}'")
